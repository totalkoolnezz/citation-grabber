...
# 5. First commit
git commit -m "Initial commit: CitationGrabber Chrome extension"

# 6. Go to github.com → New repository → name it "citation-grabber"
#    Leave it public, don't add README (you already have one)

# 7. Connect and push
git remote add origin https://github.com/YOUR_USERNAME/citation-grabber.git
git branch -M main
...
## 🧠 How it works (for AP CSP / college apps)

| Concept | Where it appears |
|---|---|
| **Abstraction** | `parseName()`, `formatMLA()`, etc. — reusable functions hide complexity |
| **Data structures** | Citation data stored as dictionaries; saved list stored as JSON array |
| **Iteration** | Loops through author lists, saved citations, session history |
| **Selection** | If/else for missing fields, different citation styles, copy confirmation |
| **Sequencing** | Scrape → parse → format → display → copy/save pipeline |
| **Input/Output** | User picks style → extension outputs formatted citation |
| **Persistence** | `chrome.storage.local` stores up to 50 citations across sessions |
